...
class DiscordAdapter(BasePlatformAdapter):
    ...
    VOICE_TIMEOUT = 300
    RUNTIME_DISCONNECT_GRACE_SECONDS = 300.0
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._runtime_disconnect_watch_task: Optional[asyncio.Task] = None
        self._runtime_disconnect_generation = 0
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async def on_ready():
                ...
                adapter_self._mark_runtime_connected()
            ...
            async def on_disconnect():
                adapter_self._mark_runtime_disconnected()
            ...
            def _on_bot_task_done(task: asyncio.Task) -> None:
                if task.cancelled() or adapter_self._disconnecting or not adapter_self._running:
                    return
                exc = task.exception()
                if exc is None:
                    return  # client.close() causes a clean exit — not a crash
                is_login_failure = isinstance(exc, discord.errors.LoginFailure)
                logger.error(
                    "[%s] Discord client task exited unexpectedly: %s",
                    adapter_self.name, exc,
                )
                adapter_self._set_fatal_error(
                    "login_failure" if is_login_failure else "bot_crash",
                    str(exc) if exc else "Client task ended",
                    retryable=not is_login_failure,
                )
                asyncio.get_event_loop().create_task(adapter_self._notify_fatal_error())
    ...
    async def disconnect(self) -> None:
        ...
        self._cancel_runtime_disconnect_watchdog()
    ...
    def _write_platform_runtime_status(
        self,
        platform_state: str,
        *,
    ...
    def _cancel_runtime_disconnect_watchdog(self) -> None:
        task = self._runtime_disconnect_watch_task
        if task and not task.done():
            task.cancel()
        self._runtime_disconnect_watch_task = None

    def _mark_runtime_connected(self) -> None:
        if self._disconnecting or self.has_fatal_error:
            return
        self._runtime_disconnect_generation += 1
        self._cancel_runtime_disconnect_watchdog()
    ...
    def _mark_runtime_disconnected(self) -> None:
        ...
        self._ready_event.clear()
        self._runtime_disconnect_generation += 1
        generation = self._runtime_disconnect_generation
        logger.warning("[%s] Discord gateway disconnected; waiting for automatic reconnect", self.name)
        self._write_platform_runtime_status(
            "retrying",
            error_code="discord_runtime_disconnect",
            error_message="Discord gateway disconnected; waiting for library reconnect",
        )
        self._cancel_runtime_disconnect_watchdog()
        self._runtime_disconnect_watch_task = asyncio.create_task(
            self._runtime_disconnect_watchdog(generation)
        )

    async def _runtime_disconnect_watchdog(self, generation: int) -> None:
        grace = max(0.0, float(self.RUNTIME_DISCONNECT_GRACE_SECONDS))
        try:
        ...
        if (
            ...
            or self._ready_event.is_set()
        ):
            return

        message = f"No Discord ready event for {grace:g}s after runtime disconnect"
        logger.error("[%s] %s; escalating to gateway reconnect", self.name, message)
        self._set_fatal_error("discord_runtime_disconnect", message, retryable=True)
    ...
    async def _run_simple_slash(
        ...
        try:
            ...
            # Interaction token expired (e.g. Discord RESUME replay after reconnect).
            # Still dispatch the command — just can't show the ephemeral "thinking..." UI.
            logger.warning(
                "[%s] slash '%s': interaction expired before defer; dispatching without UI feedback",
    ...
    async def _handle_thread_create_slash(
        ...
        try:
            ...
            logger.warning("[%s] slash '/thread': interaction expired before defer", self.name)
            return
